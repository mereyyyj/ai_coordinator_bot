...
from database import (
    ...
    save_plan, save_report, get_daily_summary, get_active_meetings, add_meeting, update_meeting, get_meeting_by_id
)
...
async def process_meeting_topic(message: types.Message, state: FSMContext):
    ...
    await message.answer("📅 Отлично! Теперь укажите <b>дату</b> (в формате <code>ДД.ММ.ГГГГ</code>, например: <code>26.10.2026</code>):")
...
async def process_meeting_time(message: types.Message, state: FSMContext):
    ...
    meeting_id = await add_meeting(topic_str, date_str, time_str)
...
# --- 1. Нажатие /reschedule: показывает кнопки со списками планерок ---
...
async def cmd_reschedule(message: types.Message, state: FSMContext):
    ...
    if not meetings:
        await message.answer("📭 Нет активных планерок для переноса.")
        return

    # Формируем клавиатуру с планерками
    keyboard_builder = []
    for m in meetings:
        btn_text = f"📌 {m['topic']} ({m['date_str']} {m['time_str']})"
        keyboard_builder.append([
            InlineKeyboardButton(text=btn_text, callback_data=f"reschedule_{m['id']}")
        ])

    kb = InlineKeyboardMarkup(inline_keyboard=keyboard_builder)

    await message.answer("🔄 <b>Выберите планерку для переноса:</b>", reply_markup=kb)


# --- 2. Обработка нажатия на кнопку выбор планерки ---
@dp.callback_query(F.data.startswith("reschedule_"))
async def process_reschedule_select(callback: CallbackQuery, state: FSMContext):
    meeting_id = int(callback.data.split("_")[1])
    meeting = await get_meeting_by_id(meeting_id)

    if not meeting:
        await callback.answer("⚠️ Эта планерка не найдена.", show_alert=True)
        return

    # Сохраняем ID планерки и тему в FSM
    await state.update_data(
        reschedule_id=meeting_id,
        reschedule_topic=meeting['topic']
    )
    
    await state.set_state(MeetingState.waiting_for_new_date)
    
    await callback.message.edit_text(
        f"🎯 Выбрана планерка: <b>{meeting['topic']}</b>\n"
        f"Текущее время: {meeting['date_str']} {meeting['time_str']}\n\n"
        "📅 Укажите <b>новую дату</b> (в формате <code>ДД.ММ.ГГГГ</code>, например: <code>15.10.2026</code>):"
    )
...
# --- 4. Ввод нового времени и перезапуск напоминания ---
...
async def process_reschedule_time(message: types.Message, state: FSMContext):
    ...
    meeting_id = user_data.get("reschedule_id")
    ...
    if meeting_id:
        await update_meeting(meeting_id, new_date_str, new_time_str)
    ...
    await message.answer("✅ Информация о переносе успешно расслана!")

    # Пересчитываем и перезаписываем напоминание за 20 минут
    try:
        meeting_datetime = datetime.strptime(f"{new_date_str} {new_time_str}", "%d.%m.%Y %H:%M")
        reminder_time = meeting_datetime - timedelta(minutes=20)

        if reminder_time > datetime.now():
            job_id = f"reminder_meeting_{meeting_id}" if meeting_id else f"reminder_{topic_str}"
            scheduler.add_job(
                send_meeting_reminder,
                'date',
                run_date=reminder_time,
            ...
            await message.answer(f"⏰ Напоминание перенесено на <b>{reminder_time.strftime('%d.%m.%Y %H:%M')}</b>.")
    except ValueError:
        await message.answer("⚠️ Неверный формат даты/времени. Напоминание не было перенесено.")
