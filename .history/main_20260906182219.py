...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if existing_user:
        await message.answer(
            ...
            f"Ты уже зарегистрирован в системе как **{existing_user['role']}**.\n\n"
    ...
    await message.answer(
        ...
        "Давай познакомимся! **Как я могу к тебе обращаться?** (Напиши свое имя и фамилию)"
    )
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Подскажи, **какая у тебя должность и чем именно ты занимаешься на работе?**\n"
        f"*(Например: Frontend-разработчик, верстаю сайты и делаю интеграцию с API)*"
    )
