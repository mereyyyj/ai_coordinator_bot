...
async def main():
    ...
    scheduler.add_job(cmd_test_digest, 'cron', hour=18, minute=30)
    scheduler.start()
...
async def cmd_meeting(message: types.Message, state: FSMContext):
    ...
    await state.set_state(MeetingState.waiting_for_topic) # pyright: ignore[reportUndefinedVariable]
    await message.answer("📝 <b>Создание планерки</b>\n\nВведите <b>тему</b> встречи:")


# Шаг 1: Получаем тему и спрашиваем время
@dp.message(MeetingState.waiting_for_topic) # pyright: ignore[reportUndefinedVariable]
async def process_meeting_topic(message: types.Message, state: FSMContext):
    ...
    await state.set_state(MeetingState.waiting_for_datetime) # pyright: ignore[reportUndefinedVariable]
...
@dp.message(MeetingState.waiting_for_datetime) # pyright: ignore[reportUndefinedVariable]
