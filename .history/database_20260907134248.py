...
async def save_daily_plan(telegram_id: int, plan_text: str):
    """Сохраняет или обновляет план на сегодня"""
    async with aiosqlite.connect(DB_NAME) as db:
        today = date.today().isoformat()
        # Используем INSERT OR REPLACE, чтобы обновлять существующую запись за сегодня
        await db.execute("""
...
async def save_daily_report(telegram_id: int, report_text: str):
...
async def get_daily_summary():
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute(query, (today, today)) as cursor:
            ...
            summary_data = []
            for row in rows:
                # Показываем только тех, кто заполнил хотя бы план или отчет
