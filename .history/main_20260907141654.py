...
# main.py
from your_file import router  # Импортируйте router из файла с кодом выше

dp.include_router(router)  # Обязательно подключаем router
...
# Если у вас используется router = Router() или dp = Dispatcher()

@dp.message(F.text)  # Перехватывает любой текст, который не подошел под команды выше
async def unknown_message_handler(message: Message):
    await message.answer(
        "Я вас не совсем понял 😅\nВыберите нужное действие из меню ниже или введите команду:",
        reply_markup=get_commands_inline_keyboard() 
    )


# 3. Обработчик нажатия на кнопку "Test"
@router.callback_query(F.data == "test")
async def process_test_button(callback: CallbackQuery):
    # Обязательно подтвeрждаем callback, чтобы убрать индикатор загрузки с кнопки
    await callback.answer()
    
    # Редактируем текущее сообщение или отправляем новое
    await callback.message.answer("Вы нажали команду Test! 🧪")


# 4. Обработчик нажатия на кнопку "Evening"
@router.callback_query(F.data == "eveningg")
async def process_evening_button(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer("Вы нажали команду Evening! 🌙")
